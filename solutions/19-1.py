...
def assemble_full_map(
    ...
    while unknown_scanners:
        for index in unknown_scanners:
            ...
            for candidate_scanner in generate_scanner_positions(map, unknown_scanner):
                if is_correct_scanner_position(map, candidate_scanner):
                    print(f"Matched scanner {index}")
                    has_matched_scanner = True
                    map = map.union(candidate_scanner)
                    break
...
def generate_scanner_positions(
    ...
    for scanner_orientation in generate_scanner_orientations(original_scanner):
        for scanner_translation in generate_scanner_translations(
            map, scanner_orientation
        ):
            yield scanner_translation
...
if __name__ == "__main__":
    ...
    full_map = assemble_full_map(scanners)
    print(len(full_map))
